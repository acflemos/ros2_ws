# laser_Tracker_4ROS.py
# ======================
# Seguidor de obstáculos com YDLIDAR X4 (4ROS) para o robô X3.
# Lógica idêntica ao laser_Tracker_a1_X3.py, mas com convenção de ângulos diferente.
#
# CONVENÇÃO DE ÂNGULOS — YDLIDAR X4 (montado de frente):
#   priorityAngle=30° → zona de prioridade: abs(angle) < 30° (frente do LiDAR = frente do robô)
#   Zona secundária: priorityAngle < abs(angle) < LaserAngle
#
# BUG — AttributeError em registerScan linha ~79:
#   `abs(angle) < self.laserAngle` — 'laserAngle' (minúsculo) não existe!
#   O atributo correto é `self.LaserAngle` (maiúsculo, declarado em __init__).
#   Este bug causa crash do nó assim que recebe o primeiro LaserScan.
#   A versão corrigida está em laser_Tracker_4ROS_R2.py (usa self.LaserAngle).
#
# Nota: não registado em setup.py — não pode ser executado via ros2 run.
#
...
class laserTracker(Node):
    ...
    def registerScan(self, scan_data):
        ...
        for i in range(len(ranges)):
            ...
            elif abs(angle) > self.priorityAngle and abs(angle) < self.LaserAngle and ranges[i] != 0.0:  # CORRIGIDO: self.laserAngle → self.LaserAngle
