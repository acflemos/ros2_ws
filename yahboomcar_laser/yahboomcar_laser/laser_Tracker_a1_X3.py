...
# =======================
# Seguidor de obstáculos com RPLIDAR A1 para o robô X3 (mecanum).
# Mantém o robô a uma distância fixa (ResponseDist) do objeto mais próximo,
# usando controlo PID em velocidade linear e angular.
#
# CONVENÇÃO DE ÂNGULOS — RPLIDAR A1 no X3 (montado para trás, yaw=π):
#   priorityAngle=30° → zona de prioridade: abs(angle) > (180-30)° = > 150°
#   (frente do robô = traseira do LiDAR)
#   Zona secundária: (180-LaserAngle) < angle < (180-priorityAngle) e simétrico
#
# Algoritmo de tracking:
#   1. Se há objetos na zona de prioridade (frente, <ResponseDist+0.5m) → usar esses
#   2. Senão, usar o mais próximo na zona secundária (flanco)
#   3. lin_pid(P=2, D=2): se perto → recuar (linear.x negativo); se longe → avançar
#   4. ang_pid(P=3, D=5): girar para alinhar com o objeto; deadzone < 0.02
#
# Parâmetros: linear=0.5, angular=1.0, LaserAngle=40°, ResponseDist=0.55m, Switch=False
#
