# laser_Tracker_4ROS_R2.py
# =========================
# Seguidor de obstáculos com YDLIDAR X4 (4ROS) para o robô R2 (Ackermann).
# Versão CORRIGIDA do laser_Tracker_4ROS.py — usa self.LaserAngle (maiúsculo).
#
# CONVENÇÃO DE ÂNGULOS — YDLIDAR X4 (montado de frente):
#   priorityAngle=30° → zona de prioridade: abs(angle) < 30° (frente do LiDAR)
#   Zona secundária: priorityAngle < abs(angle) < LaserAngle (usa self.LaserAngle — CORRETO)
#
# Esta versão corrige o bug `self.laserAngle` do laser_Tracker_4ROS.py.
# Para R2: só linear.x e angular.z efetivos.
