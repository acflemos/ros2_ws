...
# ==========================
# Desvio de obstáculos com RPLIDAR A1 para o robô X3 (mecanum).
#
# CONVENÇÃO DE ÂNGULOS — RPLIDAR A1 no X3:
#   O LiDAR é montado de frente para trás no X3 (yaw=π no TF).
#   "Frente" do robô = traseira do LiDAR = abs(angle) > 160°
#   "Direita" do robô = setor entre 160° e (180-LaserAngle)°
#   "Esquerda" do robô = setor entre -(180-LaserAngle)° e -160°
#
# Lógica: 10 casos baseados em contagens de pontos em cada setor:
#   front_warning, Left_warning, Right_warning — threshold de 10 pontos
#   ResponseDist padrão: 0.55m (zona de perigo = 1.5× = 0.825m)
#
# Parâmetros ROS2 ajustáveis em runtime:
#   linear (0.5 m/s), angular (1.0 rad/s), LaserAngle (40°),
#   ResponseDist (0.55m), Switch (False — ativa/desativa)
#
# BUG — self.Moving não inicializado:
#   JoyStateCallback usa `if self.Moving == True` mas self.Moving não é
#   definido em __init__. Causa AttributeError se o joystick enviar JoyState
#   antes de registerScan ser chamado. Solução: adicionar self.Moving = False em __init__.
#
...
class laserAvoid(Node):
    def __init__(self,name):
        ...
        self.Moving = False  # CORRIGIDO: inicialização ausente no original causava AttributeError
