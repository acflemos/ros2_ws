...
# =======================
# Modo de aviso de obstáculos: buzzer + rotação para alinhar com o obstáculo mais próximo.
# NÃO faz desvio (sem movimento linear) — apenas avisa e gira para enfrentar o obstáculo.
#
# CONVENÇÃO DE ÂNGULOS — RPLIDAR A1 no X3 (montado para trás, yaw=π):
#   Zona monitorada: abs(angle) > (180-LaserAngle)° — flancos e retaguarda do LiDAR
#   = frente e flancos do robô
#
# Comportamento:
#   Se minDist <= ResponseDist → ligar buzzer (/Buzzer=True)
#   Sempre → girar para alinhar o eixo com o obstáculo mais próximo (ang_pid P=3, D=5)
#   Deadzone angular: ang_pid_compute < 0.02 → angular.z = 0
#   linear.x sempre = 0 (não avança nem recua)
#
# Difere do laser_Avoidance por: não haver movimento de fuga, apenas aviso sonoro e rotação.
#
# Parâmetros: LaserAngle=40°, ResponseDist=0.55m, Switch=False
# Subscreve: /scan (LaserScan), /JoyState (Bool)
# Publica:   /cmd_vel (Twist — só angular), /Buzzer (Bool)
