#!/usr/bin/env python
# encoding: utf-8
# Controlo do ROSMaster X3 via joystick (rodas mecanum — suporta movimento lateral).
# Deteta automaticamente se corre na Jetson (user=root) ou num PC remoto e usa
# mapeamentos de botões diferentes para cada caso.
...
class JoyTeleop(Node):
    def __init__(self, name):
        super().__init__(name)
        self.Joy_active = False       # toggle: joystick ativo/inativo (botão L/axes[9])
        self.Buzzer_active = False
        self.RGBLight_index = 0       # cicla 0-5, regressa a 0 ao ultrapassar 6
        self.cancel_time = time.time()
        self.user_name = getpass.getuser()
        self.linear_Gear = 1          # fator de escala linear: 1/3, 2/3 ou 1
        self.angular_Gear = 1         # fator de escala angular: 1/4, 1/2, 3/4 ou 1
    ...
    def buttonCallback(self, joy_data):
        if not isinstance(joy_data, Joy):
            return
        # user=root → corre na Jetson com joystick Yahboom físico
    ...
    def user_jetson(self, joy_data):
        # Joystick Yahboom (Jetson). Mapeamento de eixos/botões do controlador físico X3.

        # axes[9]==1: toggle Joy_active + parar movimento (debounce 1s)
        if joy_data.axes[9] == 1:
            self.cancel_nav()

        # buttons[7]: ciclar cor RGB (0-5)
        ...
        # buttons[13]: ciclar marcha linear (1/3 → 2/3 → 1)
        ...
        # buttons[14]: ciclar marcha angular (1/4 → 1/2 → 3/4 → 1)
        ...
        # axes[1]=forward, axes[0]=lateral (mecanum Y), axes[2]=rotação
    ...
    def user_pc(self, joy_data):
        # Joystick genérico ligado ao PC (ex: Xbox/PS via USB).

        # axes[5]==-1: toggle Joy_active
        if joy_data.axes[5] == -1:
            self.cancel_nav()

        # buttons[5]: ciclar cor RGB
        if joy_data.buttons[5] == 1:
            if self.RGBLight_index < 6:
                # CORRIGIDO: publicar Int32() em vez de int nativo (causava TypeError em ROS2)
                self.pub_RGBLight.publish(Int32(data=self.RGBLight_index))
            else:
                self.RGBLight_index = 0
            self.RGBLight_index += 1

        # buttons[7]: toggle buzzer
        if joy_data.buttons[7] == 1:
            self.Buzzer_active = not self.Buzzer_active
            # CORRIGIDO: publicar Bool() em vez de bool nativo (causava TypeError em ROS2)
            self.pub_Buzzer.publish(Bool(data=self.Buzzer_active))

        # buttons[9]: ciclar marcha linear
        ...
        # buttons[10]: ciclar marcha angular
        ...
        # user_pc não tem guarda Joy_active — publica sempre (diferente do user_jetson)
        twist = Twist()
        twist.linear.x = xlinear_speed
        twist.linear.y = ylinear_speed
        twist.angular.z = angular_speed
    ...
    def filter_data(self, value):
        # Deadzone: ignorar inputs abaixo de 0.2 para evitar drift do joystick
        if abs(value) < 0.2:
            value = 0
        return value

    def cancel_nav(self):
        # Toggle Joy_active com debounce de 1s; para o robô ao desativar
        now_time = time.time()
        if now_time - self.cancel_time > 1:
            ...
            for _ in range(3):
                ...
                self.pub_cmdVel.publish(Twist())  # parar movimento ao toggle
