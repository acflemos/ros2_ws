// Nó de odometria para o ROSMaster R2 (cinemática Ackermann/bicicleta).
// Usa linear.y como ângulo de steering em graus (não como velocidade lateral).
// Subscreve vel_raw e integra em posição 2D. Publica /odom_raw + TF odom→base_footprint.
//
// ATENÇÃO: linear.y é REUTILIZADO como steering angle — comportamento diferente do X3/x1.
...
class OdomPublisher : public rclcpp::Node
{
   ...
   double linear_scale_y_ = 0.0;  // escala do steering angle
   ...
   double linear_velocity_y_ = 0.0;  // steering angle em graus (campo reutilizado)

   // wheelbase: distância entre eixos — crítico para o modelo Ackermann
...
private:
   void handle_vel(const std::shared_ptr<geometry_msgs::msg::Twist> msg)
   {
      ...
      linear_velocity_y_ = msg->linear.y * linear_scale_y_;  // steering angle em graus

      vel_dt_ = (current_time - last_vel_time_).seconds();
      last_vel_time_ = current_time;

      // Modelo Ackermann: calcular raio de curvatura a partir do ângulo de steering
      // R = wheelbase / tan(steer_angle) — angular.z do Twist é IGNORADO
      double steer_angle = linear_velocity_y_;
      double R = wheelbase_ / tan(steer_angle / 180.0 * M_PI);
      double angular_velocity_z = linear_velocity_x_ / R;  // rad/s calculado

      double delta_heading = angular_velocity_z * vel_dt_;
      double delta_x = (linear_velocity_x_ * cos(heading_)) * vel_dt_;  // sem componente lateral
      ...
      odom.twist.twist.linear.y = 0.0;  // Ackermann: sem movimento lateral
